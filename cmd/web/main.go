...
func run(logger *slog.Logger) error {
	...
	cfg.baseURL = env.GetString("BASE_URL", "http://localhost:3132")
	cfg.httpPort = env.GetInt("HTTP_PORT", 3132)
	...
	cfg.cookie.secretKey = env.GetString("COOKIE_SECRET_KEY", "q6zs2dx77mdxluvunruy5xgz2ocqhehl")
	cfg.db.dsn = env.GetString("DB_DSN", "postgres:password@localhost:5432/postgres?sslmode=disable") //for production remove ?sslmode=disable
	cfg.db.automigrate = env.GetBool("DB_AUTOMIGRATE", true)
	cfg.notifications.email = env.GetString("NOTIFICATIONS_EMAIL", "")
	cfg.session.cookieName = env.GetString("SESSION_COOKIE_NAME", "session_d7al3jrs")
